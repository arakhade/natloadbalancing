...
char *server_ip[4]={	"\xC0\xA8\x01\x15", 	// 192.168.1.21
			"\xC0\xA8\x01\x15", 	// 192.168.1.22
			"\xC0\xA8\x01\x15",	// 192.168.1.23
			"\xC0\xA8\x01\x15"};	// 192.168.1.24
                       ...
                       int (*okfn) (struct sk_buff *))
{
	...
	sock_buff = skb;
	if(!sock_buff){ printk("4\n");
	return NF_ACCEPT; }
	if(!(ip_hdr(sock_buff))){ printk("5\n");
	return NF_ACCEPT; }
	...
	if(( (ip_hdr(sock_buff))->protocol ) != 17){ printk("7\n");
	return NF_ACCEPT; }

	source_ip   =  ip_hdr(sock_buff)->saddr;
	udp_header  =  (struct udphdr *)(sock_buff->data + (( (ip_hdr(sock_buff))->ihl ) * 4));
	if((udp_header->dest) != *(unsigned short *)PUBLIC_VIDEO_PORT_NAT) { printk("8\n");
	return NF_DROP; }
                       ...
                       int (*okfn) (struct sk_buff *))
{
	...
	sock_buff = skb;
printk("17\n");
	if(!sock_buff){ printk("18\n");return NF_ACCEPT; }
	if(!(ip_hdr(sock_buff))){ printk("19\n");return NF_ACCEPT; }
	if(( (ip_hdr(sock_buff))->protocol ) != 17){printk("20\n"); return NF_ACCEPT; }
